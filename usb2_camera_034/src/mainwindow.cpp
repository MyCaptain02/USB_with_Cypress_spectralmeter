	...
	m_isCapturingSpectrum(false)
{
	...
	connect(ui.m_getDarkReference, &QPushButton::clicked, this, &MainWindow::getDarkReferenceLights);
...
{
	if (m_referenceParameter.empty() || m_darkreferenceParameter.empty())                   //读取参考光谱
	{
		readReferenceParameterFromFile(QApplication::applicationDirPath() + "\\reference");
		readDarkReferenceParameterFromFile(QApplication::applicationDirPath() + "\\darkreference");
	}
...
void MainWindow::analyzeSpectrum()
{
	...
	QDir dir1(appPath + "/" + "reference");
	QDir dir2(appPath + "/" + "darkreference");
...
void MainWindow::getDarkReferenceLights()
{
	//盖镜头盖
	//此时的曝光量等设置应该没有改变设置的意义，这里暂不考虑
	...
	int num = (m_maxWavelength - m_minWavelength) / m_stepOfWavelength + 1;
	QProgressDialog progress(QStringLiteral("捕获参考黑光谱"), QStringLiteral("停止捕获"), 0, num, this);
	m_progressDialog = &progress;
	progress.setWindowModality(Qt::WindowModal);      //设置模态，阻塞对父窗口的操作

	// 应用程序exe所在目录下建立频谱文件夹
	QString path = m_imageModel.getSavingPath();
	QString appPath = QCoreApplication::applicationDirPath();
	QDir dir(appPath);
	QString dirName = "darkreference";
	dir.mkdir(dirName);
	...
	int retVal = progress.exec();        //执行模式对话框

	//捕获完成之后的一些设置
	m_imageModel.captureReference(false);

	saveDarkReferenceParameterToFile(appPath + "\\" + dirName);
...
void MainWindow::getReferenceLights()
{
	...
	int num = (m_maxWavelength - m_minWavelength) / m_stepOfWavelength + 1;
	QProgressDialog progress(QStringLiteral("捕获参考白光谱"), QStringLiteral("停止捕获"), 0, num, this);
	m_progressDialog = &progress;
	progress.setWindowModality(Qt::WindowModal);      //设置模态，阻塞对父窗口的操作
...
void MainWindow::saveDarkReferenceParameterToFile(const QString& path)
{
	QSettings settings(path + "\\darkreference.ini", QSettings::IniFormat);
	QHash<int, CameraParameter>::const_iterator i = m_referenceParameter.constBegin();
	while (i != m_referenceParameter.constEnd())
	{
		settings.setValue(QString::number(i.key()) + "/exposureVal", i->m_exposureValue);
...
void MainWindow::readDarkReferenceParameterFromFile(const QString& path)
{
	QSettings settings(path + "\\darkreference.ini", QSettings::IniFormat);
	QStringList wavelengths = settings.childGroups();
	...
	while (i != wavelengths.constEnd())
	{
		...
		temp.m_exposureValue = settings.value(*i + "/exposureVal").toInt();
		temp.m_analogGain = settings.value(*i + "/analogGain").toInt();
		m_darkreferenceParameter[(*i).toInt()] = temp;
