	...
	N(0)
{
	...
	ui.m_plot->xAxis->setRange(410, 1030);
...
void SpectrumAnalysisDialog::calculateReflectivity(int x, int y)
{
	...
	{
		double ref = m_darkreferenceMat[i].at<unsigned short>(y, x) * 1.0 / m_referenceMat[i].at<unsigned short>(y, x);
	...
	//cout << m_xPlot.size() <<"  "<<m_yPlot.size() << endl;
	//QString path = m_path + "\\" + QString::number(x) + QString::number(y);

	output(m_xPlot, m_yPlot);
...
{
	//ui.m_plot->clearGraphs();             //更新图片集，选择是否保存原光谱
	//ui.m_plot->replot();
...
void SpectrumAnalysisDialog::output(QVector<double> xx, QVector<double> yy)
{
	QString path = QString::number(N)+ ".csv" ;
	QFile file(path);

	if (file.open(QFile::WriteOnly | QFile::Truncate))
	{
		QTextStream out(&file);
		out << "Wavelength" << "," << "Reflection" << endl;
		auto *t = xx.begin();
		auto *s = yy.begin();
		while ((t != xx.end()) && (s != yy.end()))
		{
			out << *t << "," << *s << endl;
			t++;
			s++;
